...
## 4차 — "원본과 100% 어긋난다" (같은 날)

marker 모델을 넣은 뒤 드리프트 경고가 **100%** 로 떴다.
100% 는 `|a−b|/b = 1` 이므로 **스트림 본문이 비었다**는 뜻이다.

익스텐션이 실제로 내보내는 이벤트를 엿들어 확인했다. 결함이 둘이었다.

### 마커가 본문보다 늦게 오면 누적분을 버렸다

```js
this.text = '';   // 사용자에게 보이는 메시지로 전환할 때
```

마커가 본문 **뒤에** 오는 스트림이 있다. 그때 이 한 줄이 이미 받은 본문을 통째로 날린다.
본문이 전부 마커 앞에 있었다면 결과는 빈 문자열 — 정확히 100%.

**고침**: 직전 대상이 *버리기로 한 것*(cot·숨김·툴)일 때만 버리고, 그 외에는 이어받는다.
...
`message_stream_complete` 와 `[DONE]` 이 **둘 다** 온다. 각각 `complete()` 를 불러
`end` 가 두 번, 따라서 `verify` 도 두 번, 빈 스트림 복구도 두 번 걸렸다.
`finished` 플래그로 한 번만 끝내게 했다.

### 덤 — 렌더 중 fiber 를 정답으로 삼던 문제

`verify` 는 `end` 400ms 뒤에 fiber 를 읽는다. 그때 원본이 아직 그리는 중이면
fiber 가 스트림보다 **짧고 스트림의 접두사**로 나온다. 실측에서 스트림 16자 / fiber 12자를 봤다.

그걸 정답으로 삼으면 화면이 오히려 짧아지고 드리프트 경고까지 뜬다.
이제 접두사 관계면 덮지 않고 900ms 뒤 **한 번만** 다시 본다.
...
누적분 이어받기를 빼면 '늦은 마커' 케이스가 즉시 실패하는 것도 같은 방식으로 확인했다.
