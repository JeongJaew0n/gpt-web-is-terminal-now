...
지금은 **자리표시자조차 안 나온다.** 이미지를 만들면 그 턴이 화면에서 통째로 사라진다.

실측: 2026-09-09 · 대화 `6a9e88a3-…` · 이미지 라이브러리 API
...
```
▤ image/png 1254×1254 · 민트 잎모자 아기 도토리 요정.png — :open 으로 원본에서 확인
```
...
얼굴이 안 보이는데, 그러면 매번 `:q` 로 원본을 열게 된다 — 터미널을 쓰는 이유가 없어진다.
...
- 부분 이미지가 온다면 같은 자리 갱신
- 안 온다면 2단계로 끝. **없는 것을 있는 척 만들지 않는다**
...
- **생성 과정이 SSE 로 오는가** (4절). 확인 전에는 3단계를 시작하지 않는다
