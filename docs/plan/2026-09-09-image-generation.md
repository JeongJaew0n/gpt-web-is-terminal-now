...
**1·2단계 구현 완료 (2026-09-10).** 새로고침 없이 그림이 뜨고,
만드는 동안 회전자가 돈다. 3단계는 취소했다(4절).
...
### 2단계 — 만드는 동안 표시가 뜨고, 끝나면 새로고침 없이 그림이 온다

실측(2026-09-10)으로 신호를 찾았다. **생성 중에는 파트가 빈 `multimodal_text` 메시지가
먼저 만들어진다.**

```
t:mult[]        ← 만드는 중          (이 순간을 잡는다)
t:mult[IMG]     ← 다 됐다            (대화 원본에서 읽는다)
```

원본은 이 메시지를 `[data-message-id]` 로 그리지 않는다 — 스트리밍 내내 DOM 에는
`user` 노드만 있었다. **DOM 수확으로는 못 본다. 스트림에서만 알 수 있다.**

- `tap.js` — `add` 의 `content_type` 이 `multimodal_text` 면 `image` 신호를 보낸다.
  스트림 상태(`current`·`messageId`)는 건드리지 않는다 — 본문 메시지가 아니다
- `store.js` — `drawing(on)` / `isDrawing()`. 켜지면 `thinkingSince` 를 비운다.
  두 줄이 겹치면 산만하다. 상한 5분을 둬 신호만 받고 끝이 안 와도 영영 돌지 않는다
- `tty.js` — `drawingRow()`. '생각 중' 과 같은 자리, 같은 회전자다
- `index.js` — **끝나면 대화 원본을 다시 읽는다.** 부분 이미지가 없으므로
  이걸 안 하면 새로고침할 때까지 아무것도 안 보인다. 원본에 파트가 채워지는 데
  시간이 걸려 간격을 늘려 가며 최대 4번 본다(0.7s → 1.4s → 2.1s)
